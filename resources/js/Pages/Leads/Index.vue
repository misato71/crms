...
/**
 * 表示件数
 * @return selected 絞り込みをしたリード情報
 */
 const selectedLeadCount = ref("All Leads");

const displayedLeads = computed(() => {
  const selected = [...props.leads];

  if (selectedLeadCount.value === "All Leads") {
    return selected;
  } else if (selectedLeadCount.value === "10件") {
    return selected.slice(0, 10);
  } else if (selectedLeadCount.value === "20件") {
    return selected.slice(0, 20);
  } else if (selectedLeadCount.value === "50件") {
...
<template>
	...
	<AuthenticatedLayout>
		...
		<div class="px-5 py-4">
			<select v-model="selectedLeadCount" class="border-none text-xs bg-gray-100 focus:border-gray-100 focus:ring-gray-100">
